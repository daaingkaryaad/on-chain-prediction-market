...
contract VerifyDeployment is Script {
    function run() external view {
        ...
        address feeVaultAddress =
            vm.envAddress("FEE_VAULT");

        address outcomeTokenAddress =
            vm.envAddress("OUTCOME_TOKEN");

        address lpTokenAddress =
            vm.envAddress("LP_TOKEN");

        GovernanceToken governanceToken =
            GovernanceToken(governanceTokenAddress);

        ProtocolGovernor governor =
            ProtocolGovernor(payable(governorAddress));

        ProtocolTimelock timelock =
            ProtocolTimelock(payable(timelockAddress));
        ...
        require(
            governor.votingPeriod() == 50_400,
            "Invalid voting period"
        );

        require(
            governor.proposalThreshold()
                == (governanceToken.totalSupply() * 1) / 100,
        ...
        require(
            ...
            "Governor missing canceller role"
        );
        ...
        require(
            ...
            "Factory deployer admin missing"
        );

        require(
            feeVault.hasRole(
                feeVault.DEFAULT_ADMIN_ROLE(),
                vm.envAddress("DEPLOYER")
            ),
        ...
        require(
            outcomeToken.hasRole(
                outcomeToken.DEFAULT_ADMIN_ROLE(),
                vm.envAddress("DEPLOYER")
            ),
        ...
        require(
            lpToken.hasRole(
                lpToken.DEFAULT_ADMIN_ROLE(),
                vm.envAddress("DEPLOYER")
            ),
            "LPToken deployer admin missing"
        );
